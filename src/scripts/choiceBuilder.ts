...
function createSingleChoice(dataset: ImgType[], datasetName: string, index: number = 0): DocumentFragment {
  ...
  Object.assign(prevButton.style, {
    ...
    clipPath:
      "polygon(40% 0%, 40% 29%, 100% 29%, 100% 70%, 40% 70%, 40% 100%, 0% 50%)",
    cursor: "pointer",
    border: "none",
  });
  ...
  function selectPrevious() {
    if (arrayIndex > 0) {
      const img = document.getElementById(`${datasetName}-img`) as HTMLImageElement;
...
export function choiceBuilder() {
  ...
  document.getElementById("main")?.appendChild(mainContainer);
}